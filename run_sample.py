...
######################################
logger.info("Step 2 - Load XTR and Index the dataset")
######################################

# Load the XTR retriever
model_name_or_path="google/xtr-base-en"
xtr = XtrRetriever(model_name_or_path=model_name_or_path, use_faiss=False, device=device)

# Build the index
xtr.build_index(chunks)

######################################
logger.info("Step 3 - Have fun")
######################################
